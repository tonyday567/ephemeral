...
-- >>> f 2 - f 1
-- 1.2189450960900354
integrate :: (Ord a, Signed a, Field a, QuotientField a Int) => a -> (a -> a) -> a -> a -> a
integrate eps f a b =
  limit_ eps (map area (iterate (/two) one))
  where
    area h = sum $ (h*) . f <$> take (floor $ (b - a) / h) (iterate (+h) a)

integrate' :: (Signed a, Field a, QuotientField a Int) => (a -> a) -> a -> a -> [a]
integrate' f a b =
...
optimize f guess = newtonRaphson guess g
  where g x = derivative2 f x / derivSecond f x
...
bisection :: (Signed a, Field a, Ord a, Epsilon a, Field b, Ord b) =>
  (a -> b) -> a -> a -> a
